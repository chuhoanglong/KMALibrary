...
interface searchBookShelfProps {
    tenKe: string,
    token: string,
}
const searchBookShelf = async (payload: searchBookShelfProps) => {
    const { tenKe, token } = payload;
    const response = await fetch(`${BASE_URL}kma/bookshelf/search`, {
        method: 'POST',
        headers: headersJwt(token),
        body: JSON.stringify({ ten_ke: tenKe }),
    });
